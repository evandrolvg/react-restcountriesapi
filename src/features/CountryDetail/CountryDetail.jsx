...
import { CountryDetailWrap } from './CountryDetailStyle';
  ...
  return (
    <CountryDetailWrap>
      ...
      <div className="coldescription">
        ...
        <div>População: {population.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</div>
        <div>
          <ButtonRound onClick={() => history.goBack()} marginleft="0" margintop="2rem">
            Voltar
          </ButtonRound>
        </div>
      </div>
